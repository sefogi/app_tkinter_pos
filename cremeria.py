...
PRODUCTS = [
    {"codigo": "P001", "nombre": "leche entera", "marca": "Rimbero", "costo": 15.00, "imagen": "images/leche_entera.webp"},
    {"codigo": "P002", "nombre": "leche deslactosada", "marca": "Rimbero", "costo": 25.00, "imagen": "images/leche_deslactosada.webp"},
    {"codigo": "P003", "nombre": "nata artesanal", "marca": "Rimbero", "costo": 18.00, "imagen": "images/nata_artesanal.webp"},
    {"codigo": "P004", "nombre": "crema", "marca": "Rimbero", "costo": 35.00, "imagen": "images/crema.webp"},
    {"codigo": "P005", "nombre": "yogurt natural", "marca": "Rimbero", "costo": 120.00, "imagen": "images/yogurt_natural.webp"},
    {"codigo": "P006", "nombre": "yogurt fresa", "marca": "Rimbero", "costo": 125.00, "imagen": "images/yogurt_fresa.webp"},
    {"codigo": "P007", "nombre": "yogurt griego", "marca": "Rimbero", "costo": 150.00, "imagen": "images/yogurt_griego.webp"},
    {"codigo": "P008", "nombre": "Leche vaporizada", "marca": "Rimbero", "costo": 16.00, "imagen": "images/leche_vaporizada.webp"},
    {"codigo": "P009", "nombre": "Helado de horchata", "marca": "Rimbero", "costo": 10.00, "imagen": "images/Helado_Horchata.webp"},
    {"codigo": "P010", "nombre": "helado de fresa", "marca": "Rimbero", "costo": 10.00, "imagen": "images/helado_fresa.webp"},
]
...
class CremeriaApp(tk.Tk):
    def __init__(self):
        ...
        # Diccionario para mantener referencias de imágenes
        self.product_images = {}
        self.cart_images = {}
    ...
    def _load_image(self, path, size):
        ...
        try:
            ...
            # Si falla, retornamos una imagen por defecto (creada sobre la marcha si no existe default)
            try:
                # intenta reutilizar default si ya fue creada
                if hasattr(self, 'default_image') and self.default_image:
                    return self.default_image
            except Exception:
                pass

            # Crear placeholder en memoria
    ...
    # ======================================================================
    # PANTALLA SELECCIONAR PRODUCTOS (con preview de imagen)
    # ======================================================================
    def show_productos(self):
        ...
        tree_frame = ttk.Frame(content, padding="10 10 10 10", style='Card.TFrame')
        tree_frame.pack(side='left', pady=10, fill='both', expand=True)
        ...
        # Panel derecho: preview de imagen + detalles
        preview_frame = ttk.Frame(content, padding="10", style='Card.TFrame')
        preview_frame.pack(side='right', fill='y', padx=(20, 0))

        ttk.Label(preview_frame, text="Vista previa", style='SubHeader.TLabel').pack(pady=(0, 10))

        # Label que contendrá la imagen
        self.preview_label = ttk.Label(preview_frame, image=self.default_image, background=COLOR_CARD)
        self.preview_label.image = self.default_image
        self.preview_label.pack(pady=5)

        # Detalles cortos
        self.preview_nombre = ttk.Label(preview_frame, text="Nombre: -", style='Body.TLabel')
        self.preview_nombre.pack(pady=(10, 2))
        self.preview_precio = ttk.Label(preview_frame, text="Precio: -", style='Body.TLabel')
        self.preview_precio.pack()
    ...
    def on_product_select(self, event):
        ...
        # Cargar imagen (y cachearla)
        img_path = p.get('imagen')
        if img_path:
            img = self._load_image(img_path, (180, 180))
        else:
            img = self.default_image

        # Mantener referencia
        self.product_images[codigo] = img
        self.preview_label.config(image=img)
        self.preview_label.image = img

        self.preview_nombre.config(text=f"Nombre: {p['nombre']}")
        self.preview_precio.config(text=f"Precio: {money(p['costo'])}")
    ...
    def show_cart(self):
        ...
        tree_frame = ttk.Frame(body, padding="10 10 10 10", style='Card.TFrame')
        ...
        # Panel derecho para preview del item seleccionado en carrito
        preview_cart = ttk.Frame(body, padding=20, style='Card.TFrame')
        preview_cart.pack(side='right', fill='y', padx=(30, 0))

        ttk.Label(preview_cart, text='Vista previa', style='SubHeader.TLabel').pack(pady=(0, 10))
        cart_preview_label = ttk.Label(preview_cart, image=self.default_image, background=COLOR_CARD)
        cart_preview_label.image = self.default_image
        cart_preview_label.pack(pady=5)

        def on_cart_select(event):#funcion de la vista previa del carrito de compras
            s = tree.selection()
            if not s:
                return
            vals = tree.item(s[0], 'values')
            nombre = vals[0]
            item = next((x for x in self.cart if x['nombre'] == nombre), None)
            if not item:
                return
            codigo = item['codigo']
            img = self._load_image(item.get('imagen', ''), (200, 200))
            self.cart_images[codigo] = img
            cart_preview_label.config(image=img)
            cart_preview_label.image = img

        tree.bind('<<TreeviewSelect>>', on_cart_select)
        ...
        button_frame = ttk.Frame(win, style='Card.TFrame')
        button_frame.pack(fill='x')

        try:
            icon_path = os.path.join(os.getcwd(), 'images', 'icono_pagar.png')
            icon_img = self._load_image(icon_path, (28, 28))
        except Exception:
            icon_img = None

        btn_cerrar = ttk.Button(button_frame,#boton cerrar carrito de compras
                               text="  Cerrar",
        ...
        if icon_img:
            btn_cerrar.image = icon_img

        btn_cerrar.pack(anchor='center', pady=5)
    ...
    def show_ticket(self):
        ...
        widths = {"nombre": 350, "cantidad": 80, "precio": 120, "subtotal": 120}
        for c in cols:
